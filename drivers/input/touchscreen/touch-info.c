			    ...
			    void *data)
{
	...
	if (unlikely(blank != FB_BLANK_POWERDOWN &&
...
{
	fb_unregister_client(&dev_to_tid(dev)->p->fb_notifier);
}
#else
...
static int get_version(struct device *dev, int *major, int *minor)
{
	struct touch_info_dev_operations *tid_ops = dev_to_tid(dev)->tid_ops;
	int ret;
	...
	ret = tid_ops->get_version(dev->parent, major ? : &ret, minor ? : &ret);
	if (unlikely(ret)) {
...
static int pin_range_to_gpio(struct pinctrl_gpio_range *range,
			     unsigned int pin)
{
	if (range->pins) {
		int i;
				    ...
				    struct regulator_dev *rdev)
{
	...
	if (rdev->open_count)
		seq_printf(s, "%-64s %-10s Min_uV   Max_uV  load_uA   %-16s use_count: %-4u enabled: %-3c\n",
...
static ssize_t hardware_info_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	size_t count = tid_hardware_info_get(buf, PAGE_SIZE);
...
static int gesture_set_capability(struct device *dev, unsigned int mask)
{
	...
	if (enable != enabled) {
		...
		if (tid_ops && tid_ops->gesture_set_capability)
			ret = tid_ops->gesture_set_capability(dev->parent,
							      enable);
		if (unlikely(ret))
			return ret;
...
static bool is_lockdown_valid(const char *buf)
{
	...
	for (i = 0; i < LOCKDOWN_INFO_SIZE; i++)
		if (buf[i])
			return true;
...
static int gesture_report_input_dev_init(struct device *dev)
{
	...
	if (!of_property_read_u32_array(dev->of_node, "touchpanel,codes", codes,
		...
		for (i = 0; i < ARRAY_SIZE(codes); i++, code++) {
			if (!codes[i])
				continue;
			dev_dbg(dev, "modify code(support_codes[%d]) from 0x%x to 0x%x\n",
...
int tid_report_key(struct touch_info_dev *tid, enum gesture_key key)
{
	...
	if (unlikely(WARN_ON(key >= GS_KEY_END)))
		return -EINVAL;
...
static int sysfs_create_link_targets(struct device *dev)
{
	...
	nr = tid_ops->sysfs_create_link_targets(dev->parent, NULL, NULL);
	if (unlikely(nr == 0)) {
		dev_info(dev, "%s: may return an invalid value\n", __func__);
		return 0;
	}
	targets = kzalloc(nr * (sizeof(*targets) + sizeof(*names)), GFP_KERNEL);
	if (!targets)
	...
	for (i = 0; i < nr; i++) {
		dev_dbg(dev, "create sysfs symlink: %s\n", names[i]);
...
static int proc_create_link_targets(struct device *dev)
{
	...
	for (i = 0; i < nr; i++) {
		...
		dev_dbg(dev, "create proc symlink: %s\n", names[i]);
...
int tid_hardware_info_get(char *buf, size_t size)
{
	...
	if(!strcmp(tid->panel_maker,"cdot"))
	{
		tid->product = "nt36525b";
		printk("touch_info This is Nt36525B cdot TP\n");
	}
	if(!strcmp(tid->panel_maker,"skyw"))
	{
		tid->product = "nt36525b";
		printk("touch_info This is Nt36525B skyw TP\n");
	}
	if(!strcmp(tid->panel_maker,"lead")){
		tid->product = "himax";
		printk("touch_info This is himax TP\n");
	}
	printk("touch_info This is not NT36525B TP\n");
...
static size_t get_firmware_name(struct device *dev, char *buf, size_t size)
{
	...
	return snprintf(buf, size, color ? "%s-%s-%s-%s.img" : "%s-%s-%s.img",
			tid->vendor, tid->product, tid->panel_maker ? : "none",
			color);
}
			     ...
			     bool force)
{
	...
	if (unlikely(ret > ARRAY_SIZE(firmware_name) - 1)) {
		dev_err(dev,
			"get firmware name fail, the buf size is too small\n");
		ret = -ENOMEM;
		goto out;
...
static ssize_t tid_regulators_get(struct device *dev, struct regulator **regs)
{
	struct device_node *np = dev->of_node;
	struct property *prop;
	size_t count = 0;
...
int tid_regulators_enable_opt(struct touch_info_dev *tid, bool enable)
{
	...
	get_device(dev);
	mutex_lock(&p->mutex);
	if (unlikely(p->regs_enabled == enable))
		goto out;
	...
	if (unlikely(!reg_count)) {
		ret = tid_regulators_get(dev, NULL);
		if (unlikely(ret <= 0))
			goto out;
		...
		ret = tid_regulators_get(dev, regs);
		if (unlikely(ret < 0)) {
			devm_kfree(dev, regs);
			goto out;
		}
	...
	for (i = 0; i < reg_count; i++) {
		...
		if (unlikely(ret)) {
			dev_err(dev, "enable regulator[%d] fail with errno: %d\n",
				i, ret);
			enable = false;
			goto disable_regs;
		}
